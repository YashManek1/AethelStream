...
fn make_labels(batch_seq: usize, vocab_size: usize) -> Vec<u32> {
    (0..batch_seq)
        .map(|i| ((i * 31 + 7) % vocab_size) as u32)
        .collect()
}
...
fn test_peak_logit_bytes_o_chunk() {
    ...
    println!(
        "\nT-CE-3 (V=128k):  MEASURED peak_logit_bytes = {} B",
        r.peak_logit_bytes
    );
    println!(
        "                  O(chunk) expected          = {} B  ({}x{} x4)",
        expected, bs, c
    );
    println!(
        "                  O(V) would be              = {} B  ({:.0}x larger)",
        full_ov,
        full_ov as f64 / r.peak_logit_bytes as f64
    );
    assert_eq!(
        r.peak_logit_bytes, expected,
        "peak {} != O(chunk) {}",
        r.peak_logit_bytes, expected
    );
    assert!(
        r.peak_logit_bytes < full_ov / 100,
        "peak ({}) must be <1% of O(V) ({})",
        r.peak_logit_bytes,
        full_ov
    );
    println!(
        "T-CE-3 PASSED — {}x reduction vs O(V)",
        full_ov / r.peak_logit_bytes
    );
}
...
fn test_determinism() {
    ...
    assert_eq!(
        r1.loss.to_bits(),
        r2.loss.to_bits(),
        "loss not bit-identical: {:.10} vs {:.10}",
        r1.loss,
        r2.loss
    );
    assert_eq!(
        max_abs_diff(&r1.grad_hidden, &r2.grad_hidden),
        0.0_f32,
...
fn test_remainder_chunk() {
    ...
    println!(
        "\nT-CE-5 (V=70 chunk=32): loss_diff={:.2e}  grad_diff={:.2e}",
        ld, gd
    );
...
fn test_single_token_sanity() {
    ...
    let w: Vec<f32> = (0..8_usize)
        .flat_map(|v| [v as f32 * 0.5, 0.0_f32])
        .collect();
    let l = vec![3_u32];
    let (d, v_size, chunk) = (2_usize, 8_usize, 4_usize);

    let max_v = 3.5_f32;
    let exps: Vec<f32> = (0..8_usize)
        .map(|v| ((v as f32 * 0.5) - max_v).exp())
        .collect();
    let sum_e: f32 = exps.iter().sum();
    ...
    println!(
        "\nT-CE-6: ref_loss={:.8}  cut={:.8}  |dl|={:.2e}",
        ref_loss, r.loss, ld
    );
    println!(
        "        grad[0]: ref={:.8}  cut={:.8}  |dg|={:.2e}",
        ref_g0, r.grad_hidden[0], g0d
    );
    ...
    assert!(
        g1d < 1e-5_f32,
        "grad[1] should be 0, got {:.2e}",
        r.grad_hidden[1]
    );

    let expected_peak = 1 * chunk * std::mem::size_of::<f32>();
    assert_eq!(
        r.peak_logit_bytes, expected_peak,
        "peak {} != {} (1xcx4)",
        r.peak_logit_bytes, expected_peak
    );
