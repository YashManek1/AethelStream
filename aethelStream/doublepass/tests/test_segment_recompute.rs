...
use doublepass::backward::{full_backward, single_layer_backward};
...
fn init_inputs(g: usize) -> Vec<Vec<f32>> {
    ...
    (0..g)
        .map(|m| {
            (0..n)
                .map(|i| ((i * 7 + m * 13) as f64 * 0.07).sin() as f32)
                .collect()
        })
        .collect()
}
...
fn reference_backward(
    ...
    for i in (0..l).rev() {
        ...
        for m in 0..g {
            let grads =
                single_layer_backward(&CFG, &model.layers[i], &all_fwd[m][i], &upstreams[m]);
            upstreams[m] = grads.d_input.clone();
            match &mut acc {
                None => acc = Some(grads),
                Some(a) => {
                    for (x, y) in a.d_rms1_w.iter_mut().zip(&grads.d_rms1_w) {
                        *x += y;
                    }
                    for (x, y) in a.d_wq.iter_mut().zip(&grads.d_wq) {
                        *x += y;
                    }
                    for (x, y) in a.d_wk.iter_mut().zip(&grads.d_wk) {
                        *x += y;
                    }
                    for (x, y) in a.d_wv.iter_mut().zip(&grads.d_wv) {
                        *x += y;
                    }
                    for (x, y) in a.d_wo.iter_mut().zip(&grads.d_wo) {
                        *x += y;
                    }
                    for (x, y) in a.d_rms2_w.iter_mut().zip(&grads.d_rms2_w) {
                        *x += y;
                    }
                    for (x, y) in a.d_wg.iter_mut().zip(&grads.d_wg) {
                        *x += y;
                    }
                    for (x, y) in a.d_wu.iter_mut().zip(&grads.d_wu) {
                        *x += y;
                    }
                    for (x, y) in a.d_wd.iter_mut().zip(&grads.d_wd) {
                        *x += y;
                    }
                }
            }
...
fn test_segment_backward_matches_reference() {
    ...
    let tol = 1e-5f32;
    println!(
        "\nT-SEG — max|Δ| per layer and parameter (tol = {:.0e}):",
        tol
    );
    ...
    macro_rules! cmp_field {
        ($li:expr, $name:expr, $ref_f:ident, $test_f:ident) => {{
            ...
            if diff >= tol {
                panic!(
                    "T-SEG FAILED layer={} {}: max|Δ|={:.3e} >= {:.3e}",
                    $li, $name, diff, tol
                );
            }
            diff
    ...
    assert_eq!(
        optimizer.call_count(),
        expected_calls,
        "hook call count: expected {expected_calls}, got {}",
        optimizer.call_count()
    );
    println!(
        "Hook calls: {} (expected {})",
...
fn test_weight_load_count() {
    ...
    println!(
        "\nWeight loads (keep_resident=true):  {} (expected {})",
        res_true.weight_loads, expected_resident
    );
    assert_eq!(
        res_true.weight_loads, expected_resident,
        "keep_resident=true: expected {expected_resident}, got {}",
        res_true.weight_loads
    );

    // keep_resident = false → 3·L
    let fwd_result2 = full_forward(&model, &inputs, &plan, false).expect("full_forward2");
    let res_false = full_backward(
        &model,
        &fwd_result2,
        &inputs,
        &upstream,
    ...
    println!(
        "Weight loads (keep_resident=false): {} (expected {})",
        res_false.weight_loads, expected_restream
    );
    assert_eq!(
        res_false.weight_loads, expected_restream,
        "keep_resident=false: expected {expected_restream}, got {}",
        res_false.weight_loads
    );
